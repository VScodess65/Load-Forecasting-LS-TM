...
import tkinter.messagebox as messagebox  # Corrected import
import numpy as np
import pandas as pd
...
def load_data():
    ...
    # Ask the user to load CSV files
    ...
    messagebox.showinfo("Data Loaded", "Load and Price data have been successfully loaded.")  # Updated messagebox usage
...
def predict_and_plot():
    ...
    # Ask for future forecasting steps
    ...
    actual_load = scaler_load.inverse_transform(scaled_load[-100:])  # Last 100 actual values
    actual_price = scaler_price.inverse_transform(scaled_price[-100:])  # Last 100 actual values
...
future_steps_label = tk.Label(app, text="Enter Number of Future Steps to Forecast:")
future_steps_label.pack(pady=5)

future_steps_entry = tk.Entry(app)
future_steps_entry.pack(pady=5)

predict_button = tk.Button(app, text="Predict and Plot", command=predict_and_plot)
